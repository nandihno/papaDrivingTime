...
final class DrivingTimeService {
    ...
    func fetchDrivingTimes(
        provider: DrivingProvider,
        googleApiKey: String,
        destinations: [DrivingDestination]? = nil
    ) async throws -> [DrivingTimeEstimate] {
        let destinations = destinations ?? DrivingDestinationStore.shared.all
    ...
    private static func parseDelayMinutes(from notices: [String]) -> Int? {
        ...
        for notice in notices {
            for pattern in patterns {
                ...
                guard let match = regex.firstMatch(in: notice, options: [], range: range),
                      match.numberOfRanges > 1,
                      let minuteRange = Range(match.range(at: 1), in: notice),
                      let minutes = Int(notice[minuteRange]) else { continue }
                return minutes
            }
        }
        return nil
    ...
    private static func noticesSuggestDelay(_ notices: [String]) -> Bool {
        let keywords = ["delay", "traffic", "congestion", "roadworks", "road work", "incident", "accident", "closure", "slow"]
...
private struct GoogleRoutesRequest: Encodable {
    ...
    struct LatLngWrapper: Encodable { let latLng: LatLng }
